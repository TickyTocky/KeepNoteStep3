...
/**
 * As in this assignment, we are working with creating RESTful web service,
 * hence annotate the class with @RestController annotation.A class annotated
 * with @Controller annotation has handler methods which returns a view.
 * However, if we use @ResponseBody annotation along with @Controller
 * annotation, it will return the data directly in a serialized format. Starting
 * from Spring 4 and above, we can use @RestController annotation which is
 * equivalent to using @Controller and @ResposeBody annotation
 */
...
public class ReminderController {
	 ...
	 * the Serialized object from request body and save the updated reminder details
	 * in a reminder table in database handle ReminderNotFoundException as well.
	 * please note that the loggedIn userID should be taken as the reminderCreatedBy
	 * for the reminder. This handler method should return any one of the status
	 ...
	 * handle ReminderNotFoundException as well. This handler method should return
	 * any one of the status messages basis on different situations: 1. 200(OK) - If
	 * the reminder found successfully. 2. 401(UNAUTHORIZED) - If the user trying to
